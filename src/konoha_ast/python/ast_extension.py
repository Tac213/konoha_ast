...
# source: https://github.com/t3rn0/ast-comments
...
def _enrich(source: str | bytes, tree: ast.AST) -> None:
    ...
    lines_iter = iter(source.splitlines(keepends=True))  # type: ignore
    tokens = tokenize.generate_tokens(lambda: next(lines_iter))  # type: ignore
    ...
    tree_intervals = _get_tree_intervals_and_update_ast_nodes(tree, source)  # type: ignore
    for c_node in comment_nodes:
        ...
        if possible_intervals_for_c_node:
            ...
            loc = -1
            for i, (low, high, _) in enumerate(sub_intervals):  # type: ignore
                if low <= c_lineno <= high or c_lineno < low:
                    loc = i
                    break

            *_, target_attr = sub_intervals[loc]  # type: ignore
        else:
            target_node = tree
            target_attr = "body"
        ...
        for attr in _CONTAINER_ATTRS:
            if items := getattr(node, attr, None):
                if not isinstance(items, abc.Iterable):
                    continue
                attr_intervals.append((*_extend_interval(_get_interval(items), source), attr))  # type: ignore
        if attr_intervals:
            ...
            if hasattr(node, "lineno") and hasattr(node, "end_lineno"):
                low, high = _extend_interval((node.lineno, node.end_lineno), source)  # type: ignore
                node.lineno = low
                ...
                high = max(node.end_lineno, max(attr_intervals)[1]) if hasattr(node, "end_lineno") else max(attr_intervals)[1]  # type: ignore
...
class _Unparser(ast._Unparser):  # pylint: disable=protected-access # type: ignore
    ...
    def visit_If(self, node: ast.If) -> None:  # pylint: disable=invalid-name
        ...
        while node.orelse:
            ...
            node = node.orelse[i]  # type: ignore
            self.fill("elif ")
            self.traverse(node.test)
            with self.block():
                self.traverse(node.body)
